...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhenLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator state chenage for second view once first image loading completes successfully")
        
        loader.completeImageLoading(at: 0)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state chenage for first view once second image loading completes successfully")
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view once second image loading completes successfully")
    }
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image for second voew while loading second image")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        let view0 = sut.simulateFeedImageViewVisible(at: 0)
        let view1 = sut.simulateFeedImageViewVisible(at: 1)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expexted no retry action for first view while loading first image")
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expexted no retry action for first view while loading second image")
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, true, "Expexted retry action for second view once second image loading completes with error")
    }
    
    func test_feedImageViewButton_isVisibleOnInvalidImageData() {
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expexted no retry action while loading image")
    ...
    func test_loadImageDataCompletion_dipatchesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
